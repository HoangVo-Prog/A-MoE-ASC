...
def run_single_train_eval(config, method=None):
    ...
    method = method or getattr(config, "fusion_method", None) or "default"
    seeds = (
    ...
    for seed in seeds:
        print(f"\n===== SINGLE seed={seed} fusion={method} =====")
        set_seed(int(seed))
        ...
        per_seed_records.append(
            {
                ...
                "train_confusion": {
                    "cm": train_cm.tolist(),
                    "cm_normalized": (
                        train_cm / np.clip(train_cm.sum(axis=1, keepdims=True), 1e-12, None)
                    ).tolist(),
                },
                "acc": float(metrics.get("acc", 0.0)),
                "f1": float(metrics.get("f1", 0.0)),
