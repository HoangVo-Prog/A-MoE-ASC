...
def build_model(*, cfg, num_labels: int):
    ...
    if head_type == "mof":
        ...
        return MoFBertConcatClassifier(
            ...
            mof_lb_mode=str(getattr(cfg, "mof_lb_mode", "switch")),
            mof_entropy_coef=float(getattr(cfg, "mof_entropy_coef", 0.001)),
            mof_router_temperature=float(getattr(cfg, "mof_router_temperature", 1.0)),
            mof_mixed_repr_norm=str(getattr(cfg, "mof_mixed_repr_norm", "layernorm")),
            mof_mixed_repr_norm_clamp=float(getattr(cfg, "mof_mixed_repr_norm_clamp", 0.0)),
            mof_residual_alpha_init=float(getattr(cfg, "mof_residual_alpha_init", 0.1)),
            mof_residual_alpha_learnable=bool(int(getattr(cfg, "mof_residual_alpha_learnable", 1))),
